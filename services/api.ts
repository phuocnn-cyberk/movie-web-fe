...
// ===== NEW: Playback Link (Bunny / Direct) =====
export type PlaybackLinkDTO = { type: "bunny-embed" | "direct"; url: string };

export const getPlaybackLink = async (movieId: string | number): Promise<PlaybackLinkDTO> => {
  const res = await api.get(`/api/movies/${movieId}/play`); // BE trả PlaybackLinkDTO
  return res.data;
};
...
export const createPaypalOrder = async (data: CreatePaypalOrderData): Promise<PaypalOrderResponse> => {
  try {
    ...
    if (typeof response.data === "string") {
      return { orderId: "temp-order-id", approvalUrl: response.data, status: "created" };
    }
